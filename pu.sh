...
spin_stop(){ [ -n "$SPIN_PID" ]&&{ kill "$SPIN_PID" 2>/dev/null;wait "$SPIN_PID" 2>/dev/null;SPIN_PID="";}
[ -t 2 ]&&{ printf '\r\033[K' >&2;tput cnorm >&2 2>/dev/null||true;}
}
_kill_tree(){ local p="$1" c;[ -n "$p" ]&&[ "$p" -gt 0 ] 2>/dev/null||return 0;if command -v pgrep >/dev/null 2>&1;then for c in $(pgrep -P "$p" 2>/dev/null);do _kill_tree "$c"
done
fi;kill -TERM "$p" 2>/dev/null||true;sleep 0.2;if command -v pgrep >/dev/null 2>&1;then for c in $(pgrep -P "$p" 2>/dev/null);do _kill_tree "$c";done
...
each_tool_use(){
printf '%s' "$1"|awk -v m="$2" 'BEGIN{RS="\001"}{s=$0
  while((p=index(s,m))>0){
...
think_param(){
case "$PROVIDER:$MODEL" in anthropic:claude-opus-4-7*|anthropic:claude-opus-4-6*|anthropic:claude-sonnet-4-6*|anthropic:claude-opus-4-5*)echo ',"effort":"'$EFFORT'","thinking":{"type":"adaptive"}';;
...
};local out="" rc=0;case "$tool_name" in bash)local cmd;cmd=$(jp "$input" command);_tool bash "$cmd";local tf;tf=$(mktemp);printf '%s\n' "$cmd" >"$tf"
...
out=$(sed -n "$off,\$p" "$fp");elif [ -n "$lim" ];then out=$(head -n "$lim" "$fp");else out=$(cat "$fp");fi
fi;else out="Error: file not found: $fp";rc=1;fi
;; write)local fp;fp=$(jp "$input" path);case "$fp" in -*)fp="./$fp";esac
local ct;ct=$(jp "$input" content;printf x);ct=${ct%x};_tool write "$(_p "$fp")";[ -L "$fp" ]&&{ local l;l=$(readlink "$fp")||{ out="Error reading symlink: $fp";rc=1;}
[ $rc -eq 0 ]&&case "$l" in /*)fp=$l;; *)fp="$(dirname "$fp")/$l";esac
};if [ $rc -ne 0 ];then :;elif case "$ct" in *"to=functions."*|*"<|channel|>"*|*"<|start|>assistant"*)true;;*)false;esac;then
out="Error: content contains harmony channel markers (likely model CoT leakage); refusing write";rc=1;elif ! mkdir -p "$(dirname "$fp")" 2>/dev/null;then
out="Error creating directory for $fp";rc=1;else local tmp mode um;tmp=$(mktemp "$(dirname "$fp")/.pu.XXXXXX")||{ out="Error creating temp file";rc=1;}
if [ $rc -eq 0 ];then if [ -e "$fp" ];then mode=$(stat -f %Lp "$fp" 2>/dev/null||stat -c %a "$fp" 2>/dev/null||true);else um=$(umask);mode=$(printf '%03o' $((0666&~0$um)));fi
[ -n "$mode" ]&&chmod "$mode" "$tmp" 2>/dev/null||true;printf '%s' "$ct" >"$tmp"&&mv "$tmp" "$fp"&&out="Wrote to $fp"||{ rm -f "$tmp";out="Error writing $fp";rc=1;}
fi;fi
;; edit)local fp;fp=$(jp "$input" path);case "$fp" in -*)fp="./$fp";esac
local old;old=$(jp "$input" oldText;printf x);old=${old%x};local new;new=$(jp "$input" newText;printf x);new=${new%x};_tool edit "$(_p "$fp")";[ -L "$fp" ]&&{ local l
l=$(readlink "$fp")||{ out="Error reading symlink: $fp";rc=1;}
[ $rc -eq 0 ]&&case "$l" in /*)fp=$l;; *)fp="$(dirname "$fp")/$l";esac
};if [ $rc -ne 0 ];then :;elif [ -z "$old" ];then out="Error: oldText must not be empty";rc=1
elif case "$new" in *"to=functions."*|*"<|channel|>"*|*"<|start|>assistant"*)true;;*)false;esac;then
out="Error: newText contains harmony channel markers (likely model CoT leakage); refusing edit";rc=1;elif [ -f "$fp" ];then local tmp mode
tmp=$(mktemp "$(dirname "$fp")/.pu.XXXXXX")||{ out="Error creating temp file";rc=1;}
...
mv "$tmp" "$fp";out="Edited $fp";}||{ rc=$?;rm -f "$tmp"
...
MSGS="";return 1;}
_replay(){ [ "$INTERACTIVE" = 1 ]&&[ -f "$LOG" ]||return;info "Last messages from $LOG:"
tail -200 "$LOG"|awk '/"t":"start"/{b=""}{b=b $0 "\n"}END{printf "%s",b}'|while IFS= read -r l;do t=$(jp "$l" t);c=$(jp "$l" c)
case "$t" in start)printf '\033[36m>\033[0m %s\n' "$c";; response)printf '%s\n' "$c";; tool_call)printf '⏺ %s\n' "$c";; error|max_steps)err "$c";esac
done >&2;}
append(){ [ "$MSGS" = "[]" ]&&MSGS="[$1]"||MSGS=$(printf '%s' "$MSGS"|sed 's/]$//')",$1]";}
RA='"role":"assistant"' RU='"role":"user"' RT='"role":"tool"';TOKEN_IN=0 TOKEN_OUT=0 COST_USD=0
track_tokens(){ local u a b;u=$(jp "$1" usage);case "$PROVIDER" in anthropic)a=$(jp "$u" input_tokens);b=$(jp "$u" output_tokens);; *)a=$(jp "$u" input_tokens)
b=$(jp "$u" output_tokens);[ -z "$a" ]&&a=$(jp "$u" prompt_tokens);[ -z "$b" ]&&b=$(jp "$u" completion_tokens);esac
...
_status(){ local d;case "$PWD" in "$HOME"/*)d="~/${PWD#"$HOME"/}";; *)d="$PWD";esac
printf '%s%s ↑%s ↓%s' "$d" "$(_branch)" "$(_fmtk "$TOKEN_IN")" "$(_fmtk "$TOKEN_OUT")";[ "$COST" = 1 ]&&printf ' $%.3f' "$COST_USD"
printf ' %s (%s) %s' "$(_ctxp)" "$PROVIDER" "$MODEL";[ "$EFFORT_OK" = 1 ]&&printf ' • %s' "$EFFORT";}
_ctx_entries(){
printf '%s' "$1"|awk '
...
_ctx_valid(){ local x="$1" cap="$2";case "$x" in \[*\]);; *)return 1;esac
[ ${#x} -le "$cap" ]||return 1;case "$x" in *',,'*|'[,'*|*',]'*)return 1;esac
return 0;}
...
_ctx_last_resort(){ local cap="$1" f="${2:-}" msg x;msg="[Earlier context was compacted locally due to size.${f:+ Focus: $f.} Continue from the latest user request.]"
x='[{"role":"user","content":"'$(json_escape "$msg")'"}]';[ ${#x} -le "$cap" ]&&printf '%s' "$x"||printf '[]';}
_ctx_tail_start(){ printf '%s\n' "$1"|awk -v k="$2" '{a[NR]=$0;l[NR]=length($0)}END{s=0;for(i=NR;i>1;i--){s+=l[i];if(s>k)break}print i+1}';}
_ctx_adjust_start(){ local o="$1" n="$2" c="$3" h guard=0;while :;do h=$(printf '%s\n' "$o"|sed -n "${c}p");case "$h" in *reasoning*)break;;
*tool_result*|*function_call_output*|*'"type":"function_call"'*)c=$((c-1));; *)break;esac
guard=$((guard+1));[ "$c" -le 2 ]||[ "$guard" -gt 20 ]&&break;done
[ "$c" -lt 2 ]&&c=$((n-2));printf '%s' "$c";}
_ctx_sanitize_openai(){ local m="$1" cap="$2" o f new;[ "$PROVIDER" = openai ]||{ printf '%s' "$m";return;}
case "$m" in *'"type":"function_call_output"'*|*'"type"'*'"function_call_output"'*);; *)printf '%s' "$m";return;esac
o=$(_ctx_entries "$m");f=$(_ctx_filter_openai_pairs "$o");new=$(printf '[%s]' "$(printf '%s\n' "$f"|tr '\n' ','|sed 's/,$//')")
[ "$f" = "$o" ]&&case "$new" in *'large or malformed tool '*'omitted during compaction'*);; *)printf '%s' "$m";return;esac
if _ctx_valid "$new" "$cap";then log 0 compact "old=${#m} new=${#new} cap=$cap mode=sanitize-openai-pairs";printf '%s' "$new";else printf '%s' "$m";fi
}
_ctx_sanitize_anthropic(){ local m="$1" cap="$2" o f new;[ "$PROVIDER" = anthropic ]||{ printf '%s' "$m";return;}
case "$m" in *'"type":"tool_result"'*|*'"type"'*'"tool_result"'*|*'"type":"tool_use"'*|*'"type"'*'"tool_use"'*);; *)printf '%s' "$m";return;esac
o=$(_ctx_entries "$m");f=$(_ctx_filter_anthropic_pairs "$o");new=$(printf '[%s]' "$(printf '%s\n' "$f"|tr '\n' ','|sed 's/,$//')")
[ "$f" = "$o" ]&&case "$new" in *'large or malformed tool '*'omitted during compaction'*);; *)printf '%s' "$m";return;esac
if _ctx_valid "$new" "$cap";then log 0 compact "old=${#m} new=${#new} cap=$cap mode=sanitize-anthropic-pairs";printf '%s' "$new";else printf '%s' "$m";fi
}
...
[ -z "$f" ]&&[ ${#m} -le "$cap" ]&&{ _ctx_sanitize_pairs "$m" "$cap";return;}
info "Compacting (${#m}b > ${cap}b)${f:+ focus: $f}";o=$(_ctx_entries "$m")
case "$PROVIDER" in openai)o=$(_ctx_filter_openai_pairs "$o");;anthropic)o=$(_ctx_filter_anthropic_pairs "$o");esac;n=$(printf '%s\n' "$o"|wc -l|tr -d ' ');[ "$n" -lt 6 ]&&{
[ ${#m} -le "$cap" ]&&printf '%s' "$m"||_ctx_last_resort "$cap" "$f";return;}
half=$((cap/2));[ "$kb" -gt "$half" ]&&kb=$half;[ "$kb" -lt 2000 ]&&kb=2000;c=$(_ctx_tail_start "$o" "$kb");c=$(_ctx_adjust_start "$o" "$n" "$c");a=$(printf '%s\n' "$o"|sed -n 1p)
...
req='[{"role":"user","content":"'$(json_escape "$p")'"}]';res=$(call_api "$req");parse_response "$res";compaction_summary_text=$TX;if [ -z "$compaction_summary_text" ];then
err "Summarization failed; using local compaction memory";compaction_summary_text=$(_ctx_local_memory "$mid" "$f");mode=local;else mode=normal;fi
if [ "$mode" = local ];then s='{"role":"user","content":"[Earlier compacted locally:\n'$(json_escape "$compaction_summary_text")']"}';else
s='{"role":"user","content":"[Earlier compacted memory:\n'$(json_escape "$compaction_summary_text")']"}';fi
while :;do c=$(_ctx_tail_start "$o" "$kb");c=$(_ctx_adjust_start "$o" "$n" "$c");r=$(printf '%s\n' "$o"|sed -n "$c,${n}p"|tr '\n' ','|sed 's/,$//')
...
[ "$kb" -le 2000 ]&&break;kb=$((kb/2));[ "$kb" -lt 2000 ]&&kb=2000;done
if [ "$mode" = local ];then
localnote=$(printf '%s\n' "$o"|sed -n "2,${n}p"|awk '{if(length($0)>4000)print "[large transcript entry omitted: "length($0)" chars]";else print}'|tail -160)
compaction_summary_text=$(_ctx_local_memory "$localnote" "$f");s='{"role":"user","content":"[Earlier compacted locally:\n'$(json_escape "$compaction_summary_text")']"}';fi
...
localnote=$(printf '%s\n' "$o"|sed -n "2,${n}p"|awk '{if(length($0)>4000)print "[large transcript entry omitted: "length($0)" chars]";else print}'|tail -160)
localnote=$(_ctx_local_memory "$localnote" "$f");s='{"role":"user","content":"[Earlier compacted locally:\n'$(json_escape "$localnote")']"}';new=$(printf '[%s]' "$s")
if _ctx_valid "$new" "$cap";then log 0 compact "old=${#m} new=${#new} cap=$cap mode=emergency";printf '%s' "$new";return;fi
new=$(_ctx_last_resort "$cap" "$f");log 0 compact "old=${#m} new=${#new} cap=$cap mode=last-resort";printf '%s' "$new";}
load_context(){ local dir ctx f;dir=$(pwd);ctx="";while [ "$dir" != "/" ];do for f in AGENTS.md CLAUDE.md;do
[ -f "$dir/$f" ]&&ctx="$ctx
$(cat "$dir/$f")"||true
done;dir=$(dirname "$dir");done
...
run_task(){ _STATE=busy;local task="$1";case "$task" in '!'*)local r;"$RUNSH" -c "${task#!}" 2>&1& _CHILD=$!;wait "$_CHILD"||r=$?;_CHILD=0;[ "$_STATE" = idle ]&&return 130
return "${r:-0}";esac
_ensure_key||return 1;local task_esc;task_esc=$(json_escape "$task");[ -z "$MSGS" ]&&load||true
[ -n "$MSGS" ]&&[ "$MSGS" != "" ]&&append "{\"role\":\"user\",\"content\":\"$task_esc\"}"||MSGS="[{\"role\":\"user\",\"content\":\"$task_esc\"}]";log 0 start "$task"
local step=0 empty_final=0 ctx_retry=0;while [ "$step" -lt "$MAX_STEPS" ];do step=$((step+1));MSGS=$(trim_context "$MSGS");_STATE=busy;local resp="" retry=0
while [ $retry -lt 3 ];do local rf cr=0;rf=$(mktemp);spin_start "$(_status)";call_api "$MSGS" >"$rf" 2>&1& _CHILD=$!;wait "$_CHILD"||cr=$?;_CHILD=0;resp=$(cat "$rf");rm -f "$rf"
spin_stop;[ -n "${AGENT_DEBUG_API:-}" ]&&mkdir -p "$AGENT_DEBUG_API" 2>/dev/null&&{ printf '%s' "$MSGS" >"$AGENT_DEBUG_API/input-$step-$retry.json"
...
case "$(printf '%s' "$em"|tr A-Z a-z)" in *incorrect*api*key*|*invalid*api*key*|*unauthorized*|*authentication*)err "API: $em";return 1;; *invalid*body*|*parse*json*)err "API: $em"
return 1;; *model*not*found*|*model*does*not*exist*|*model*not*exist*|*access*model*)err "API: $em";err "Try /model MODEL";return 1;;
*context*|*token*limit*|*too*large*)[ "$ctx_retry" = 0 ]&&{ ctx_retry=1;err "Context full; compacting and retrying";MSGS=$(trim_context "$MSGS" "recover from context overflow")
continue;}
esac;[ -n "$api_err" ]&&[ "$api_err" != "null" ]&&[ "$api_err" != "" ]&&{ err "API: $em";retry=$((retry+1));sleep $((retry*3));continue;}
break;done
...
[ "$PIPE" = 0 ]&&[ "$INTERACTIVE" = 1 ]&&_say "$TX"||printf '%s\n' "$TX";log "$step" response "$TX";append "{\"role\":\"assistant\",\"content\":\"$(json_escape "$TX")\"}"
info "done · $(_status)";save;return 0;else err "Parse failed";dbg "$resp";log "$step" error "Parse fail";return 1;fi
done;err "Max steps ($MAX_STEPS)";info "stopped · $(_status)";log "$step" max_steps "Limit";return 1;}
_tpl(){ for d in .pi/prompts "$HOME/.pi/agent/prompts";do [ -f "$d/$1.md" ]&&{ cat "$d/$1.md";return;}
done;echo "$1";}
_skill(){ for d in .pi/skills .agents/skills "$HOME/.pi/agent/skills" "$HOME/.agents/skills";do [ -f "$d/$1/SKILL.md" ]&&{ info "Loaded skill: $1"
SYSTEM="$SYSTEM
$(cat "$d/$1/SKILL.md")"
return;}
done;err "Skill not found: $1";}
_export(){ local out="${1:-session.md}";printf '# Session Export\n\n' >"$out";[ -f "$LOG" ]&&while IFS= read -r line;do local t;t=$(jp "$line" t);local c;c=$(jp "$line" c)
...
[ -n "$os" ]&&stty "$os" 2>/dev/null||stty echo 2>/dev/null;printf '\n' >&2;k=$(_clean_key "$k");[ -z "$k" ]&&{ err "No key entered";exit 1;}
...
tp=$(_tpl "$cn");[ "$tp" != "$cn" ]&&{ info "Template: $cn";run_task "$tp";return 0;}
err "Unknown command: $1";return 0;esac
return 1;}
TASK=""
if [ "$PIPE" = 1 ]&&[ ! -t 0 ];then IN=$(cat);[ $# -gt 0 ]&&TASK=$([ -n "$IN" ]&&printf '%s\n%s' "$IN" "$*"||printf '%s' "$*")||TASK="$IN";else [ $# -gt 0 ]&&TASK="$*"||{
[ ! -t 0 ]&&TASK=$(cat);}
